...
def _edit_choose_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💰 Narx", callback_data="cfe:price")],
        [InlineKeyboardButton("📊 Kanal", callback_data="cfe:channel")],
...
async def on_new_confirm_cp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    op_id = int((context.user_data.get("operator") or {}).get("id") or 0)
...
# ✅ main.py import qiladigan NOM: on_channel_pick
...
# ✅ main.py import qiladigan NOM: on_group_pick
async def on_group_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("8) 💰 Narx yozing. Masalan: 450")
    return CF_PRICE
...
async def on_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "back":
        await q.edit_message_text("⬅️ Orqaga qaytdingiz. /tasdiq ni qayta bosing.", reply_markup=None)
        return ConversationHandler.END
    ...
    if not (item_type and size and isinstance(qty, int) and qty > 0 and isinstance(price_uzs, int) and price_uzs > 0):
        await q.edit_message_text("❌ Ma’lumotlar to‘liq emas (nimaligi/razmer/son/narx).")
        return ConversationHandler.END

    if not sc_meta or not gp_meta:
        await q.edit_message_text("❌ Kanal yoki Группа tanlanmagan.")
        return ConversationHandler.END
    ...
    try:
        ...
        await q.edit_message_text("✅ Buyurtma MoySklad + kanalga yuborildi.")

        # Kanalga yuborish (agar bor bo‘lsa)
        if CONFIRM_CHAT_ID:
            caption = (
...
async def on_edit_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if key == "brand":
        ...
        d["phone_plus"] = _normalize_phone_uz(val.strip())
    elif key == "item":
        ...
        dd = _digits_only(val)
        if not dd:
            await update.message.reply_text("❌ Soni noto‘g‘ri.")
            return CF_EDIT_VALUE
        d["qty"] = int(dd)
    elif key == "price":
        dd = _digits_only(val)
