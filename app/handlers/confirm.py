...
async def on_new_confirm_cp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (not op_id) or (not cp) or (not cp.get("meta")):
        await update.message.reply_text("❌ Kontragent topish/yaratishda xatolik. Qayta urinib ko‘ring.")
        return ConversationHandler.END
...
async def on_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "back":
        # review xabarini saqlab qolish shart emas
        try:
        ...
        await context.bot.send_message(
            ...
            text="⬅️ Orqaga qaytdik. /tasdiq ni bosing.",
    ...
    if not cp_meta:
        await q.edit_message_text("❌ Kontragent meta yo‘q.")
        return ConversationHandler.END
    ...
    try:
        ...
        # ✅ DUPLICATE bo‘lmasligi uchun: review xabarini o‘chiramiz, keyin 1ta final yuboramiz
        try:
            await q.message.delete()
        except Exception:
            pass
